#include "main.h"

const color_t COLOR_RED = { 236, 100, 75 };
const color_t COLOR_GREEN = { 135, 211, 124 };
const color_t COLOR_LIGHTGREEN = { 0, 255, 0 };
const color_t COLOR_YELLOW = { 255, 255, 0 };
const color_t COLOR_BLACK = { 0, 0, 0 };
const color_t COLOR_BACKGROUND = { 242, 241, 239 };
const color_t COLOR_BLUEGRAY = {17, 42, 82};
const color_t COLOR_LIGHTGRAY = {111, 128, 155};
const color_t COLOR_GRAY = { 182, 196, 209};
const color_t COLOR_ORANGE = { 255, 165, 0};
const color_t COLOR_DARKGRAY = { 105, 105, 105};
const color_t COLOR_BLUE = { 0, 204, 204};
const color_t COLOR_LIGHTBLUE = { 0, 153, 255};
const color_t COLOR_OCHRE = { 244, 164, 96};
const color_t COLOR_VIOLET = { 153, 0, 204};